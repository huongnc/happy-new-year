...
function handleStateChange(state, prevState) {
  ...
  const wasRunning =
    !prevState.paused && !prevState.menuOpen && prevState.config.autoLaunch;
...
const WISH_MESSAGES = [
  "Năm mới thật dịu dàng với bé nhé 🌸",
  "Mọi cố gắng của bé đều sẽ nở hoa ✨",
  "Bé cứ việc tỏa sáng, thế giới cứ để anh lo 🌍",
  "Chúc bé yêu của anh luôn bình yên ❤️",
  "Mong bé lúc nào cũng ăn ngon, ngủ kỹ 🌙",
  "Ra tết lên 50 kýyyy 🌙",
  "Công việc thuận lợi, đi đến nơi về đến chốn 🚀",
  "Bé chỉ cần là chính mình, vì bé là tuyệt nhất 😊",
  "Chúc bé một năm mới không còn những đêm mệt mỏi ☁️",
  "Năm mới mình cùng nhau đi khắp nơi nhé ✈️",
  "Bé cười một cái là cả năm may mắn luôn 🧧",
  "Hy vọng mọi điều bé chọn đều xứng đáng 🌟",
  "Dù chuyện gì xảy ra, anh luôn đứng cạnh bé 👫",
  "Chúc bé đủ mạnh mẽ để theo đuổi ước mơ 💪",
  "Yêu bé bằng tất cả sự tôn trọng và dịu dàng 🍬",
  "Năm mới, tình mình lại càng thêm 'dính' 🍯",
];
...
function spawnWishMessage() {
  ...
  inner.addEventListener("pointermove", (e) => {
    ...
    globalWishRotationX = Math.max(
      -60,
      Math.min(60, dragStartRotationX - dy * 0.3),
    );
...
function hasActiveWishes() {
  ...
  const wishWrappers = appNodes.wishesLayer.querySelectorAll(
...
function startSequence() {
  ...
  if (
    sequencesSinceFinale >= SEQUENCES_BEFORE_FINALE &&
    (!hasActiveWishes() || wishesStopped)
  ) {
    isFinalePhase = true;
    seqRandomFastShell();
